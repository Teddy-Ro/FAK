...
void MyDayTasks::loadTaskData(const QString &taskData)
{
    // Очищаем текущие задачи
    QLayoutItem* child;
    while ((child = ui->buttonsLayout->takeAt(0)) != nullptr) {
        delete child->widget();
        delete child;
    }
    ...
    if (!taskData.isEmpty()) {
        // Здесь можно добавить парсинг taskData и создание соответствующих задач
        // Пока просто создаем одну задачу с полученными данными
        QWidget* taskWidget = new QWidget();
        QHBoxLayout* taskLayout = new QHBoxLayout(taskWidget);

        QCheckBox* checkbox = new QCheckBox();
        QPushButton* taskButton = new QPushButton(taskData);
        QToolButton* optionsButton = new QToolButton();
        QToolButton* deleteButton = new QToolButton();

        // Настройка виджетов как в createButtonFromInput()
        // ...
...
void MyDayTasks::createTaskFromData(const QString &taskData)
{
...
void MyDayTasks::setDatabaseName(const QString &name)
{
    if (db.isOpen()) {
        db.close();
    }
    ...
    QString dbPath = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation);
    QDir().mkpath(dbPath);
    db.setDatabaseName(dbPath + "/task_" + name + ".db");

    if (!db.open()) {
        qDebug() << "Error opening task database:" << db.lastError().text();
    } else {
        QSqlQuery query;
        query.exec("CREATE TABLE IF NOT EXISTS tasks ("
                  "id INTEGER PRIMARY KEY AUTOINCREMENT, "
